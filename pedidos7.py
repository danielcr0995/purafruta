#!/usr/bin/python3
'''nueva funcion de eliminar pedido y cambiar pedido que elimina el pedido cambiado'''
    ...
    for line in file:
        ...
        for i in range(len(productos)):
            ...
            if 'Notas' in productos[i]:
                ...
                cantidades[i]=float(cantidades[i])
                #print(i,productos[i],len(productos),cantidades[i], len(cantidades))
                precios[i]=float(precios[i])
                costos[i]=float(costos[i])     
                productos[i]=productos[i].replace(" '","").replace("'","")
            #print(nombre,cantidades)
...
#        if pedidocliente==cliente.codigocliente or (nombre[0]==cliente.nombre and nombre[1]==cliente.apellido): #or codigoClienteIngresado==:
            ...
            while prod!=000:
                if productos[prod-1]=='Extras':
                    ...
                    nota=input("Nota: ")
                    if nota!='xxx':
                        cantidadesfl.append(1)
                        productosfl.append(productos[prod-1]+': '+nota)
                        preciosfl.append(0)
                        costosfl.append(0)
                        posproductos.append(prod-1)
                else:
...
def imprimirPedidoInd4(nombre,apellido,urbanizacion,direccion,sector,tamanox,tamanoy,documento,productos,cantidad,precios,movex,movey,numeropedido): #,posinicialx,posinicialy
    ...
    for i in range(len(productos)):
        ...
        if productos[i]=='Canasta basica':
            ...
            cantidadescanasta=[2      ,1       ,2         ,4       ,2      ,1        ,1     ,  1,       1.7]
            documento.drawString(30+movex*tamanox/1,tamanoy-20-movey*tamanoy/2,str(numeropedido))
            #documento.drawString(-30+movex*tamanox/1,tamanoy-20-movey*tamanoy/2,str(numeropedido))
            sep=tlinea-len(str(productos[i]))-len(str(precios[i]*cantidad[i]))-1-20-len(str(cantidad[i]))-2#-1 de $ y -2 de espacios
            #print(sp)
    ...
    for i in range(len(productos)):
        if 'Notas' in productos[i]:
            ...
            for i in range((len(nota)//letraspagina)+1):
                posImprimirproductos-=15
                documento.drawString(30+movex*tamanox/1,posImprimirproductos-movey*tamanoy/2,nota[i*letraspagina:(i+1)*letraspagina])
...
#for i in range (len(productos)):
#    cantidades.append(0)
def eliminarPedido(dataframepedidos,archivopedidos,archivoproductos):
    numeropedido=int(input('Numero de pedido: '))
    pedido=dataframepedidos.iloc[numeropedido]
    print(pedido)
    fechai=dt.datetime(2020,1,1)
    fechaf=dt.datetime.today()
    fechainiciopedidos=dt.datetime.timestamp(fechai)
    fechafinpedidos=dt.datetime.timestamp(fechaf)
    ...
    for prod in productos:
        if pedido[prod]!=0:
            cantidadesf.append(pedido[prod])
            posproducto=productos.index(prod)
            productosf.append(productos[posproducto])
            preciosf.append(productos[posproducto])
            costosf.append(productos[posproducto])

    file=open(archivopedidos,'w')
    file.write('fecha;nombre;apellido;codigocliente;telefono;sector;urbanizacion;direccion;productos;cantidades;precios;costos;pago\n')

    for pedidofl in pedidos:
        ffl=pedidofl.fecha
        fechafl=dt.datetime.fromtimestamp(ffl).date()
        #print(fechafl==fecha_pedidodf)

        if fecha_pedidodf!=fechafl:
            if pedidofl.nombre!=nombre:
                if pedidofl.apellido!=apellido:
                    if pedidofl.codigocliente!=codigocliente:
                        if pedidofl.productos!=productosf:
                            if pedidofl.cantidades!=cantidadesf:
            
                                file.write(str(pedidofl.fecha)+";"+pedidofl.nombre+";"+pedidofl .apellido+";"+pedidofl.codigocliente+";"+pedidofl.telefono+";"+pedidofl.sector+";"+pedidofl.urbanizacion+
                                    ";"+pedidofl.direccion+";")
                                file.write(str(pedidofl.productos)+";"+str(pedidofl.cantidades)+";"+str(pedidofl.precios)+";"+str(pedidofl.costos)+";"+pedidofl.pago+"\n")

def convertirFecha(pedido):
    '''une la fecha del pedido con minutos par que la fecha sea un 
    objeto con datetime y no solo date par poder convertirle a timestamp'''

    fechadf=pedido['Fecha']

    f_min=dt.datetime.min.time()
    f=dt.datetime.combine(fechadf,f_min)
    fecha=dt.datetime.timestamp(f)
    return fecha

def cambiarPedido2(dataframepedidos,archivopedidos,archivoproductos):
    numeropedido=int(input('Numero de pedido: '))
    pedido=dataframepedidos.iloc[numeropedido]
    print(pedido)    
    file=open(archivopedidos,"a")
    ...
    eliminarPedido(dataframepedidos,archivopedidos,archivoproductos)
