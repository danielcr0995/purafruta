...
def eliminarPedido(dataframepedidos,archivopedidos,archivoproductos):
    ...
    for pedidofl in pedidos:
        if pedidofl.nombre!=nombre and pedidofl.apellido!=apellido and pedidofl.codigocliente!=codigocliente and pedidofl.productos!=productos and pedidofl.cantidades!=cantidadesf:
    ...
    while prod!=000:
        if productos[prod-1]=='Extras':
            ...
            if cantidadextra!=0:
                ...
                costoextra=float(input('Costo: '))
                #cantidadextra=float(input('Cantidad: '))
                cantidades.append(cantidadextra)
                productos.append(prodextra.title())
                precios.append(precioextra)
                costos.append(costoextra)
                posproductos.append(prod-1)
        elif productos[prod-1]=='Notas':
            nota=input("Nota: ")
            if nota!='xxx':
                cantidades.append(1)
                productos.append(productos[prod-1]+': '+nota)
                precios.append(0)
                costos.append(0)
                posproductos.append(prod-1)
        else:
            cantidad=float(input('Cantidad '+productos[prod-1].upper()+": "))
            if cantidad!=0:
                if productos[prod-1] in productos:
                    posproducto=productos.index(productos[prod-1])
                    cantidades[posproducto]+=cantidad
                else:
                    cantidades.append(cantidad)
                    productos.append(productos[prod-1])
                    precios.append(precios[prod-1])
                    costos.append(costos[prod-1])
                    posproductos.append(prod-1)
        prod=int(input('Producto: '))
